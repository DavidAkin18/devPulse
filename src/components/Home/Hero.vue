...
export default {
  ...
  computed: {
    currentCountry() {
      return this.countries[this.countryCounter]; // Dynamically updates the text
    }
  },
  methods: {
    changeCountry() {
      this.countryCounter = (this.countryCounter + 1) % this.countries.length;
    }
  },
  mounted() {
    setInterval(this.changeCountry, 2000);
  }
};
</script>
